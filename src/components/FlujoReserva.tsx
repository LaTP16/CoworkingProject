...
import { SEDES_DATABASE, SpaceCategory } from "@/data/sedesData";
...
export default function FlujoReserva({
  ...
  const activeSpaceCategory = currentSede.spaces.find(
    (s) => s.id === selectedSpaceId
  );
  const totalCapacity = activeSpaceCategory?.count || 1;
        ...
        {selectedDay !== null && (
          <div className="bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-sm mb-8 animate-fade-in">
            <div className="flex items-center justify-between pb-4 mb-6 border-b border-gray-100">
              ...
              {selectedSpaceId && (
                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5" /> {activeSpaceCategory?.name} (Capacidad: {activeSpaceCategory?.count})
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {currentSede.spaces.map((espacio) => {
                const isSelected = selectedSpaceId === espacio.id;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-3 border-t border-white/20 flex flex-col gap-2.5">
                      ...
                      <button
                        onClick={() => handleSelectSpace(espacio.id)}
                        type="button"
                      ...
                      >
                        {isSelected ? "Espacio Seleccionado ✓" : "Seleccionar para reservar"}
                      </button>
                    </div>
                  </div>
                );
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Elige tus horas en la lista vertical y revisa el resumen al costado derecho ($15/hr)
                  </p>
                </div>
              </div>
                ...
                <div className="pt-3 border-t border-blue-100 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-extrabold text-blue-600">
                      ${horasSeleccionadas.length * 15}.00
                    </span>
                  </div>
